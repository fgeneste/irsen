...
public class PrintfFormat {
    ...
    public String sprintf(Object[] o) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (c == '\0') sb.append(cs.getLiteral()); else if (c == '%') sb.append("%"); //$NON-NLS-1$
            else {
                ...
                if (o[i] instanceof Byte) sb.append(cs.internalsprintf(((Byte) o[i]).byteValue())); else if (
                    o[i] instanceof Short
                ) sb.append(cs.internalsprintf(((Short) o[i]).shortValue())); else if (o[i] instanceof Integer) sb.append(
                    cs.internalsprintf(((Integer) o[i]).intValue())
                ); else if (o[i] instanceof Long) sb.append(cs.internalsprintf(((Long) o[i]).longValue())); else if (
                    o[i] instanceof Float
                ) sb.append(cs.internalsprintf(((Float) o[i]).floatValue())); else if (o[i] instanceof Double) sb.append(
                    cs.internalsprintf(((Double) o[i]).doubleValue())
                ); else if (o[i] instanceof Character) sb.append(cs.internalsprintf(((Character) o[i]).charValue())); else if (
                    o[i] instanceof String
                ) sb.append(cs.internalsprintf((String) o[i])); else sb.append(cs.internalsprintf(o[i]));
    ...
    public String sprintf(Object x) throws IllegalArgumentException {
        ...
        while (e.hasMoreElements()) {
            ...
            if (c == '\0') sb.append(cs.getLiteral()); else if (c == '%') sb.append("%"); //$NON-NLS-1$
                ...
                if (x instanceof Byte) sb.append(cs.internalsprintf(((Byte) x).byteValue())); else if (x instanceof Short) sb.append(
                    cs.internalsprintf(((Short) x).shortValue())
                ); else if (x instanceof Integer) sb.append(cs.internalsprintf(((Integer) x).intValue())); else if (
                    x instanceof Long
                ) sb.append(cs.internalsprintf(((Long) x).longValue())); else if (x instanceof Float) sb.append(
                    cs.internalsprintf(((Float) x).floatValue())
                ); else if (x instanceof Double) sb.append(cs.internalsprintf(((Double) x).doubleValue())); else if (
                    x instanceof Character
                ) sb.append(cs.internalsprintf(((Character) x).charValue())); else if (x instanceof String) sb.append(
                    cs.internalsprintf((String) x)
                ); else sb.append(cs.internalsprintf(x));
            }
        }
        return sb.toString();
    ...
    private class ConversionSpecification {
        ...
        ConversionSpecification(String fmtArg) throws IllegalArgumentException {
            ...
            if (fmtArg.length() == 0) throw new IllegalArgumentException("Control strings must have positive" + " lengths."); //$NON-NLS-1$ //$NON-NLS-2$
            if (fmtArg.charAt(0) == '%') {
                fmt = fmtArg;
                pos = 1;
                setArgPosition();
                ...
                if (setConversionCharacter()) {
                    ...
                    } else throw new IllegalArgumentException("Malformed conversion specification=" + fmtArg); //$NON-NLS-1$
                } else throw new IllegalArgumentException("Malformed conversion specification=" + fmtArg); //$NON-NLS-1$
            } else throw new IllegalArgumentException("Control strings must begin with %."); //$NON-NLS-1$
        }
        ...
        String internalsprintf(int s) throws IllegalArgumentException {
            ...
            switch (conversionCharacter) {
                case 'd':
                case 'i':
                    if (optionalh) s2 = printDFormat((short) s); else if (optionall) s2 = printDFormat((long) s); else s2 = printDFormat(s);
                    break;
                case 'x':
                case 'X':
                    if (optionalh) s2 = printXFormat((short) s); else if (optionall) s2 = printXFormat((long) s); else s2 = printXFormat(s);
                    break;
                case 'o':
                    if (optionalh) s2 = printOFormat((short) s); else if (optionall) s2 = printOFormat((long) s); else s2 = printOFormat(s);
                    break;
                case 'c':
                case 'C':
        ...
        String internalsprintf(long s) throws IllegalArgumentException {
            ...
            switch (conversionCharacter) {
                case 'd':
                case 'i':
                    if (optionalh) s2 = printDFormat((short) s); else if (optionall) s2 = printDFormat(s); else s2 = printDFormat((int) s);
                    break;
                case 'x':
                case 'X':
                    if (optionalh) s2 = printXFormat((short) s); else if (optionall) s2 = printXFormat(s); else s2 = printXFormat((int) s);
                    break;
                case 'o':
                    if (optionalh) s2 = printOFormat((short) s); else if (optionall) s2 = printOFormat(s); else s2 = printOFormat((int) s);
                    break;
                case 'c':
                case 'C':
                    s2 = printCFormat((char) s);
        ...
        String internalsprintf(double s) throws IllegalArgumentException {
            ...
            switch (conversionCharacter) {
                ...
                default:
                    throw new IllegalArgumentException(
                        "Cannot " +
                        "format a double with a format using a " + //$NON-NLS-1$ //$NON-NLS-2$
                        conversionCharacter +
                        " conversion character."
                    ); //$NON-NLS-1$
            }
        ...
        String internalsprintf(String s) throws IllegalArgumentException {
            String s2 = ""; //$NON-NLS-1$
            if (conversionCharacter == 's' || conversionCharacter == 'S') s2 = printSFormat(s); else throw new IllegalArgumentException(
                "Cannot " +
                "format a String with a format using a " + //$NON-NLS-1$ //$NON-NLS-2$
                conversionCharacter +
                " conversion character."
            ); //$NON-NLS-1$
            return s2;
        ...
        String internalsprintf(Object s) {
            String s2 = ""; //$NON-NLS-1$
            if (conversionCharacter == 's' || conversionCharacter == 'S') s2 =
                printSFormat(s.toString()); else throw new IllegalArgumentException(
        ...
        private char[] fFormatDigits(double x) {
            ...
            if (x > 0.0) sx = Double.toString(x); else if (x < 0.0) {
            ...
            int rPos = sx.indexOf('.');
            if (rPos != -1) n1In = rPos; else if (ePos != -1) n1In = ePos; else n1In = sx.length();
            if (rPos != -1) {
                if (ePos != -1) n2In = ePos - rPos - 1; else n2In = sx.length() - rPos - 1;
            } else n2In = 0;
            if (ePos != -1) {
                int ie = ePos + 1;
                expon = 0;
                if (sx.charAt(ie) == '-') {
                    for (++ie; ie < sx.length(); ie++) if (sx.charAt(ie) != '0') break;
                    if (ie < sx.length()) expon = -Integer.parseInt(sx.substring(ie));
                } else {
                    if (sx.charAt(ie) == '+') ++ie;
                    for (; ie < sx.length(); ie++) if (sx.charAt(ie) != '0') break;
                    if (ie < sx.length()) expon = Integer.parseInt(sx.substring(ie));
                }
            }
            int p;
            if (precisionSet) p = precision; else p = defaultDigits - 1;
            char[] ca1 = sx.toCharArray();
            char[] ca2 = new char[n1In + n2In];
            char[] ca3, ca4, ca5;
            for (j = 0; j < n1In; j++) ca2[j] = ca1[j];
            i = j + 1;
            for (k = 0; k < n2In; j++, i++, k++) ca2[j] = ca1[i];
            if (n1In + expon <= 0) {
                ca3 = new char[-expon + n2In];
                for (j = 0, k = 0; k < (-n1In - expon); k++, j++) ca3[j] = '0';
                for (i = 0; i < (n1In + n2In); i++, j++) ca3[j] = ca2[i];
            } else ca3 = ca2;
            boolean carry = false;
            if (p < -expon + n2In) {
                if (expon < 0) i = p; else i = p + n1In;
                carry = checkForCarry(ca3, i);
                if (carry) carry = startSymbolicCarry(ca3, i - 1, 0);
            }
            if (n1In + expon <= 0) {
                ca4 = new char[2 + p];
                if (!carry) ca4[0] = '0'; else ca4[0] = '1';
                if (alternateForm || !precisionSet || precision != 0) {
                    ca4[1] = '.';
                    for (i = 0, j = 2; i < Math.min(p, ca3.length); i++, j++) ca4[j] = ca3[i];
                    for (; j < ca4.length; j++) ca4[j] = '0';
                }
            } else {
                if (!carry) {
                    if (alternateForm || !precisionSet || precision != 0) ca4 = new char[n1In + expon + p + 1]; else ca4 =
                        new char[n1In + expon];
                    j = 0;
                } else {
                    if (alternateForm || !precisionSet || precision != 0) ca4 = new char[n1In + expon + p + 2]; else ca4 =
                        new char[n1In + expon + 1];
                    ca4[0] = '1';
                    j = 1;
                }
                for (i = 0; i < Math.min(n1In + expon, ca3.length); i++, j++) ca4[j] = ca3[i];
                for (; i < n1In + expon; i++, j++) ca4[j] = '0';
                if (alternateForm || !precisionSet || precision != 0) {
                    ca4[j] = '.';
                    j++;
                    for (k = 0; i < ca3.length && k < p; i++, j++, k++) ca4[j] = ca3[i];
                    for (; j < ca4.length; j++) ca4[j] = '0';
                }
            }
            int nZeros = 0;
            if (!leftJustify && leadingZeros) {
                ...
                if (thousands) {
                    ...
                    int xdp = xlead;
                    for (; xdp < ca4.length; xdp++) if (ca4[xdp] == '.') break;
            ...
            for (i = 0; i < nZeros; i++, j++) ca5[j] = '0';
            for (i = 0; i < ca4.length; i++, j++) ca5[j] = ca4[i];

            int lead = 0;
            if (ca5[0] == '+' || ca5[0] == '-' || ca5[0] == ' ') lead = 1;
            int dp = lead;
            for (; dp < ca5.length; dp++) if (ca5[dp] == '.') break;
        ...
        private String fFormatString(double x) {
            ...
            if (Double.isInfinite(x)) {
                if (x == Double.POSITIVE_INFINITY) {
                    if (leadingSign) ca6 = "+Inf".toCharArray(); //$NON-NLS-1$
                    else if (leadingSpace) ca6 = " Inf".toCharArray(); //$NON-NLS-1$
                    else ca6 = "Inf".toCharArray(); //$NON-NLS-1$
                } else ca6 = "-Inf".toCharArray(); //$NON-NLS-1$
                noDigits = true;
            } else if (Double.isNaN(x)) {
                if (leadingSign) ca6 = "+NaN".toCharArray(); //$NON-NLS-1$
                else if (leadingSpace) ca6 = " NaN".toCharArray(); //$NON-NLS-1$
                else ca6 = "NaN".toCharArray(); //$NON-NLS-1$
                noDigits = true;
        ...
        private char[] eFormatDigits(double x, char eChar) {
            ...
            if (precisionSet) p = precision; else p = defaultDigits - 1;
            if (rPos != -1 && ePos != -1) ca1 = (sx.substring(0, rPos) + sx.substring(rPos + 1, ePos)).toCharArray(); else if (
                rPos != -1
            ) ca1 = (sx.substring(0, rPos) + sx.substring(rPos + 1)).toCharArray(); else if (ePos != -1) ca1 =
                sx.substring(0, ePos).toCharArray(); else ca1 = sx.toCharArray();
            boolean carry = false;
            int i0 = 0;
            if (ca1[0] != '0') i0 = 0; else for (i0 = 0; i0 < ca1.length; i0++) if (ca1[i0] != '0') break;
            if (i0 + p < ca1.length - 1) {
                ...
                if (carry) {
                    ...
                    for (i = i0, j = i0 + 1; j < p + 1; i++, j++) ca2[j] = ca1[i];
                    expon++;
                    ca1 = ca2;
                }
            }
            if (Math.abs(expon) < 100 && !optionalL) eSize = 4; else eSize = 5;
            if (alternateForm || !precisionSet || precision != 0) ca2 = new char[2 + p + eSize]; else ca2 = new char[1 + eSize];
            if (ca1[0] != '0') {
                ca2[0] = ca1[0];
                j = 1;
            } else {
                for (j = 1; j < (ePos == -1 ? ca1.length : ePos); j++) if (ca1[j] != '0') break;
            ...
            for (k = 0; k < p && j < ca1.length; j++, i++, k++) ca2[i] = ca1[j];
            for (; i < ca2.length - eSize; i++) ca2[i] = '0';
            ca2[i++] = eChar;
            if (expon < 0) ca2[i++] = '-'; else ca2[i++] = '+';
            ...
            if (!leftJustify && leadingZeros) {
                ...
                if (thousands) {
                    ...
                    int xdp = xlead;
                    for (; xdp < ca2.length; xdp++) if (ca2[xdp] == '.') break;
                    xThousands = (xdp - xlead) / 3;
                }
            ...
            for (k = 0; k < nZeros; j++, k++) ca3[j] = '0';
            for (i = 0; i < ca2.length && j < ca3.length; i++, j++) ca3[j] = ca2[i];

            int lead = 0;
            if (ca3[0] == '+' || ca3[0] == '-' || ca3[0] == ' ') lead = 1;
            int dp = lead;
            for (; dp < ca3.length; dp++) if (ca3[dp] == '.') break;
            int nThousands = dp / 3;
            // Localize the decimal point.
            ...
            if (thousands && nThousands > 0) {
                ...
                for (; i < ca3.length; i++, k++) ca4[k] = ca3[i];
            }
            return ca4;
        }
        ...
        private boolean checkForCarry(char[] ca1, int icarry) {
            ...
            if (icarry < ca1.length) {
                if (ca1[icarry] == '6' || ca1[icarry] == '7' || ca1[icarry] == '8' || ca1[icarry] == '9') carry = true; else if (
                    ca1[icarry] == '5'
                ) {
                    int ii = icarry + 1;
                    for (; ii < ca1.length; ii++) if (ca1[ii] != '0') break;
                    carry = ii < ca1.length;
                    if (!carry && icarry > 0) {
                        carry =
                            (
        ...
        private String eFormatString(double x, char eChar) {
            ...
            if (Double.isInfinite(x)) {
                if (x == Double.POSITIVE_INFINITY) {
                    if (leadingSign) ca4 = "+Inf".toCharArray(); //$NON-NLS-1$
                    else if (leadingSpace) ca4 = " Inf".toCharArray(); //$NON-NLS-1$
                    else ca4 = "Inf".toCharArray(); //$NON-NLS-1$
                } else ca4 = "-Inf".toCharArray(); //$NON-NLS-1$
                noDigits = true;
            } else if (Double.isNaN(x)) {
                if (leadingSign) ca4 = "+NaN".toCharArray(); //$NON-NLS-1$
                else if (leadingSpace) ca4 = " NaN".toCharArray(); //$NON-NLS-1$
                else ca4 = "NaN".toCharArray(); //$NON-NLS-1$
                noDigits = true;
            ...
            if (fieldWidthSet) {
                ...
                if (leftJustify) {
                    ...
                    if (nBlanks > 0) {
                        ...
                        for (i = 0; i < ca4.length; i++) ca5[i] = ca4[i];
                        for (j = 0; j < nBlanks; j++, i++) ca5[i] = ' ';
                    }
                } else if (!leadingZeros || noDigits) {
                    nBlanks = fieldWidth - ca4.length;
                    if (nBlanks > 0) {
                        ca5 = new char[ca4.length + nBlanks];
                        for (i = 0; i < nBlanks; i++) ca5[i] = ' ';
                        for (j = 0; j < ca4.length; i++, j++) ca5[i] = ca4[j];
                    }
                    ...
                    if (nBlanks > 0) {
                        ...
                        for (int k = 0; k < nBlanks; i++, k++) ca5[i] = '0';
                        for (; j < ca4.length; i++, j++) ca5[i] = ca4[j];
                    }
                }
            }
            return ca5;
        ...
        private String printEFormat(double x) {
            if (conversionCharacter == 'e') return eFormatString(x, 'e'); else return eFormatString(x, 'E');
        }
        ...
        private String printGFormat(double x) {
            ...
            if (Double.isInfinite(x)) {
                ...
                // a digit, trim it, too.
                if (!alternateForm) {
                    if (expon >= -4 && expon < precision) sy = fFormatString(x).trim(); else sy = sx.substring(0, ePos);
                    i = sy.length() - 1;
                    for (; i >= 0; i--) if (sy.charAt(i) != '0') break;
                    if (i >= 0 && sy.charAt(i) == '.') i--;
                    if (i == -1) sz = "0"; //$NON-NLS-1$
                    else if (!Character.isDigit(sy.charAt(i))) sz = sy.substring(0, i + 1) + "0"; //$NON-NLS-1$
                    else sz = sy.substring(0, i + 1);
                    if (expon >= -4 && expon < precision) ret = sz; else ret = sz + sx.substring(ePos);
                } else {
                    if (expon >= -4 && expon < precision) ret = fFormatString(x).trim(); else ret = sx;
                }
                // leading space was trimmed off during
                // construction
                if (leadingSpace) if (x >= 0) ret = " " + ret; //$NON-NLS-1$
                ca4 = ret.toCharArray();
            }
        ...
        private String printDFormat(String sx) {
            ...
            if (
                sx.equals("0") && precisionSet && precision == 0
            ) sx = ""; //$NON-NLS-1$ //$NON-NLS-1$
            if (!neg) {
            ...
            if (leadingSign) n++; else if (leadingSpace) n++;
            n += nBlanks;
            n += nLeadingZeros;
            n += sx.length();
            char[] ca = new char[n];
            if (leftJustify) {
                if (neg) ca[i++] = '-'; else if (leadingSign) ca[i++] = '+'; else if (leadingSpace) ca[i++] = ' ';
                char[] csx = sx.toCharArray();
                jFirst = neg ? 1 : 0;
                for (int j = 0; j < nLeadingZeros; i++, j++) ca[i] = '0';
                for (int j = jFirst; j < csx.length; j++, i++) ca[i] = csx[j];
                for (int j = 0; j < nBlanks; i++, j++) ca[i] = ' ';
            } else {
                if (!leadingZeros) {
                    ...
                    if (neg) ca[i++] = '-'; else if (leadingSign) ca[i++] = '+'; else if (leadingSpace) ca[i++] = ' ';
                    for (int j = 0; j < nBlanks; j++, i++) ca[i] = '0';
                }
                for (int j = 0; j < nLeadingZeros; j++, i++) ca[i] = '0';
                char[] csx = sx.toCharArray();
                jFirst = neg ? 1 : 0;
                for (int j = jFirst; j < csx.length; j++, i++) ca[i] = csx[j];
            }
        ...
        private String printXFormat(short x) {
            String sx = null;
            if (x == Short.MIN_VALUE) sx = "8000"; //$NON-NLS-1$
            else if (x < 0) {
                String t;
                if (x == Short.MIN_VALUE) t = "0"; //$NON-NLS-1$
                else {
            ...
            } else sx = Integer.toString((int) x, 16);
        ...
        private String printXFormat(long x) {
            String sx = null;
            if (x == Long.MIN_VALUE) sx = "8000000000000000"; //$NON-NLS-1$
            else if (x < 0) {
        ...
        private String printXFormat(int x) {
            String sx = null;
            if (x == Integer.MIN_VALUE) sx = "80000000"; //$NON-NLS-1$
            else if (x < 0) {
        ...
        private String printXFormat(String sx) {
            ...
            if (leftJustify) {
                ...
                for (int j = 0; j < nLeadingZeros; j++, i++) ca[i] = '0';
                char[] csx = sx.toCharArray();
                for (int j = 0; j < csx.length; j++, i++) ca[i] = csx[j];
                for (int j = 0; j < nBlanks; j++, i++) ca[i] = ' ';
            } else {
                if (!leadingZeros) for (int j = 0; j < nBlanks; j++, i++) ca[i] = ' ';
                if (alternateForm) {
                    ca[i++] = '0';
                    ca[i++] = 'x';
                }
                if (leadingZeros) for (int j = 0; j < nBlanks; j++, i++) ca[i] = '0';
                for (int j = 0; j < nLeadingZeros; j++, i++) ca[i] = '0';
                char[] csx = sx.toCharArray();
                for (int j = 0; j < csx.length; j++, i++) ca[i] = csx[j];
            }
        ...
        private String printOFormat(short x) {
            String sx = null;
            if (x == Short.MIN_VALUE) sx = "100000"; //$NON-NLS-1$
            else if (x < 0) {
            ...
            } else sx = Integer.toString((int) x, 8);
        ...
        private String printOFormat(long x) {
            String sx = null;
            if (x == Long.MIN_VALUE) sx = "1000000000000000000000"; //$NON-NLS-1$
            else if (x < 0) {
        ...
        private String printOFormat(int x) {
            String sx = null;
            if (x == Integer.MIN_VALUE) sx = "20000000000"; //$NON-NLS-1$
            else if (x < 0) {
        ...
        private String printOFormat(String sx) {
            ...
            if (leftJustify) {
                ...
                if (leadingZeros) for (i = 0; i < nBlanks; i++) ca[i] = '0'; else for (i = 0; i < nBlanks; i++) ca[i] = ' ';
                for (int j = 0; j < nLeadingZeros; j++, i++) ca[i] = '0';
                char[] csx = sx.toCharArray();
                for (int j = 0; j < csx.length; j++, i++) ca[i] = csx[j];
            }
        ...
        private String printSFormat(String x) {
            ...
            if (nPrint >= x.length()) n += x.length(); else n += nPrint;
            char[] ca = new char[n];
            int i = 0;
            if (leftJustify) {
                if (nPrint >= x.length()) {
                ...
                for (int j = 0; j < width - nPrint; j++, i++) ca[i] = ' ';
            } else {
                for (i = 0; i < width - nPrint; i++) ca[i] = ' ';
                if (nPrint >= x.length()) {
                    char[] csx = x.toCharArray();
                    for (int j = 0; j < x.length(); i++, j++) ca[i] = csx[j];
                } else {
                    char[] csx = x.substring(0, nPrint).toCharArray();
                    for (int j = 0; j < nPrint; i++, j++) ca[i] = csx[j];
                }
            }
            return new String(ca);
        }
            ...
            if (pos < fmt.length() && fmt.charAt(pos) == '.') {
                ...
                if ((pos < fmt.length()) && (fmt.charAt(pos) == '*')) {
                    ...
                    while (pos < fmt.length()) {
                        char c = fmt.charAt(pos);
                        if (Character.isDigit(c)) pos++; else break;
                    }
                    if (pos > firstPos + 1) {
        ...
        private void setFlagCharacters() {
            ...
            for (; pos < fmt.length(); pos++) {
                ...
                } else if (c == '#') alternateForm = true; else if (c == '0') {
