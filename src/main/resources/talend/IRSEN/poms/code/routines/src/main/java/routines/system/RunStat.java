...
public class RunStat implements Runnable {
    ...
    public void sendMessages() {
        ...
        for (String curKey : keysList) {
            ...
            if (jobStat == JOBDEFAULT) { //it mean job is running here for connection status, not a good name
                ...
                if (jobStat == JOBSTART) {
                    jobStatStr = jobName + "|" + "start job" + "|" + itemId + "|" + simpleDateFormat.format(new Date(sb.getStartTime()));
                } else if (jobStat == JOBEND) {
                    jobStatStr = jobName + "|" + "end job" + "|" + itemId + "|" + simpleDateFormat.format(new Date(sb.getEndTime()));
                }
    ...
    public synchronized boolean log(
        ...
        String iterateId,
        String connectionUniqueName,
        int mode,
        int nbLine,
        String sourceNodeId,
        String sourceNodeLabel,
        String sourceNodeComponent,
        String targetNodeId,
        String targetNodeLabel,
        String targetNodeComponent,
        String lineType
    ) {
     ...
     * work for avoiding the 65535 issue
     */
    public synchronized void updateStatOnConnection(
        Map<String, Object> resourceMap,
        String iterateId,
        int mode,
        int nbLine,
     ...
     * work for avoiding the 65535 issue
     */
    public synchronized void updateStatAndLog(
        boolean execStat,
        boolean enableLogStash,
        Map<String, Object> resourceMap,
        String iterateId,
    ...
    public synchronized boolean updateLog(String iterateId, int mode, int nbLine, String... informationGroup) {
        ...
        for (int i = 0; i < informationGroup.length; i++) {
            if ((i % 7) == 0) {
                ...
                emit |=
                    log(
                        informationGroup[i] + iterateId,
                        mode,
                        nbLine,
                        informationGroup[i + 1],
                        informationGroup[i + 2],
                        informationGroup[i + 3],
                        informationGroup[i + 4],
                        informationGroup[i + 5],
                        informationGroup[i + 6]
                    );
            }
        }
        return emit;
     ...
     * work for avoiding the 65535 issue
     */
    public synchronized void updateStatOnConnectionAndLog(
        ...
        for (String connectionUniqueName : connectionUniqueNames) {
            ...
            java.util.concurrent.atomic.AtomicInteger stats = (java.util.concurrent.atomic.AtomicInteger) concurrentHashMap.get(
                connectionUniqueName + iterateLoop
            );
