...
public class StringHandling {
    ...
    public static String CHANGE(String oldStr, String regex, String replacement) {
        if (oldStr == null || regex == null || replacement == null) return oldStr; else return oldStr.replaceAll(regex, replacement);
    }
    ...
    public static int INDEX(String string, String element) {
        if (string == null || element == null) return -1; else return string.indexOf(element);
    }
    ...
    public static String LPAD(String first_string, int length, String second_string) {
        ...
        if (isVacant(second_string)) {
            ...
            for (int len = result.length(); len < length - OriginLength; result.append(second_string)) len = result.length();
    ...
    public static Integer INSTR(byte[] string, byte[] search_value, Integer start, Integer occurrence) {
        ...
        if (defaultStart > 0) {
            ...
            for (int i = 0; i < total; i++) revers[max - i] = string[i];
            for (int i = 0; i < search_value.length; i++) revers_search[search_value.length - 1 - i] = search_value[i];
        ...
        for (int i = defaultStart - 1; i <= max; i++) {
            ...
            if (search_value.length > 1) {
                ...
                if (i <= max) {
                    ...
                    for (int k = 1; j + 1 <= max && k < search_value.length && string[j + 1] == search_value[k]; j++, k++);
