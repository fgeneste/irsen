      ...
      <table class="table table-striped" aria-describedby="mandats">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('irsenApp.mandat.mandatAncien')">Mandat Ancien</span></th>
            <th scope="row"><span v-text="$t('irsenApp.mandat.mandatEnCours')">Mandat En Cours</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="mandat in mandats" :key="mandat.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="mandat.mandatAncien">
                <router-link :to="{ name: 'MandatAncienView', params: { mandatAncienId: mandat.mandatAncien.id } }">{{
                  mandat.mandatAncien.id
                }}</router-link>
              </div>
            </td>
            <td>
              <div v-if="mandat.mandatEnCours">
                <router-link :to="{ name: 'MandatEnCoursView', params: { mandatEnCoursId: mandat.mandatEnCours.id } }">{{
                  mandat.mandatEnCours.id
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
